...
def recognize_the_passport(
    ...
    if not is_authorized:
        ...
        return RecognitionResult(status='error', result=f"Несанкционированный запрос")
    
    try:
        url: str = decode_image_url(image)
    except Exception as e:
        logger.error(f"Не удалось получить ссылку на файл, или ссылка указана некоректно: {str(e)}")
        return RecognitionResult(status='error', result=f"Не удалось получить ссылку на файл, или ссылка указана некоректно: {str(e)}")
